...
import { SileteUser, SileteOrg } from "../types/auth.types";
 ...
 * until fetchMySchool() resolves the real school name.
 */
export async function login(email: string, password: string): Promise<SileteUser> {
  ...
  const user: SileteUser = {
    ...
    email,
    displayName: email, // placeholder until fetchMySchool() resolves school_name
    role,
    orgId: org_id,
    isActive: true,
  };
  ...
  useAuthStore.getState().setSession({
    ...
    // TODO(backend): no onboarding-complete signal exists yet on
    // Organization. Stubbed as incomplete until a real field/endpoint
    // exists — revisit once that's confirmed.
...
/**
 * GET /orgs/my-school — admin-only. Use this after login when the UI
 * needs the school's display name, short_code, or slug, none of which
 * are in the login response. Also backfills `user.displayName` with
 * the real school name once it resolves.
 *
 * Will 403 if called for a staff-role user (allow_admin_only on the
 * backend) — only call this when user.role === "admin".
 */
export async function fetchMySchool(): Promise<SileteOrg> {
  ...
  const { user, setOrg, setSession, accessToken, onboardingStatus } = useAuthStore.getState();
  setOrg(org);
  if (user && accessToken) {
    setSession({
 ...
 * /resend-verification, no /auth prefix). Also: this version of the
