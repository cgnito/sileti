...
class AuthContext:
    """Unified identity payload for both school admins and staff accounts."""
...
def create_verification_token(email: str) -> str:
    """Issue a 24-hour token strictly for email verification."""
...
def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)) -> AuthContext:
    """Decode the bearer token and return the caller's AuthContext."""
    try:
    ...
    # Check school admin table first
