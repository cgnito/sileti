...
# LIST CLASSES (Access: All Users)
@router.get("/", response_model=List[ClassResponse])
def list_classes(
    db: Session = Depends(get_db),
    current_user: models.User = Depends(security.get_current_user)
):
    classes = db.query(models.SchoolClass).filter(
        models.SchoolClass.org_id == current_user.org_id
    ).order_by(models.SchoolClass.level.asc()).all()
    return classes
...
def delete_class(
    ...
    if student_count:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Cannot delete a class that contains students. Please move or remove students first."
        )
