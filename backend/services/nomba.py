"""
Nomba payment gateway client.

Handles token lifecycle, API requests, checkout order creation,
and transaction verification. No FastAPI routing concerns live here.
"""
import os
import time
import logging
...
# In-memory token cache — prevents redundant auth round-trips.
_token_cache: dict = {
    "access_token": None,
    "refresh_token": None,
    "expires_at": 0,
...
def _resolve_checkout_callback_url() -> str:
    """
    Resolve the public callback URL Nomba redirects to after checkout.
    Prefers an explicit env override, falls back to the app's frontend origin.
    """
...
def get_nomba_access_token(force_refresh: bool = False) -> str:
    """
    Retrieve a valid Nomba access token, refreshing it when close to expiry.
    The token is cached in-process to avoid redundant auth requests.
    """
...
def make_nomba_request(method: str, endpoint: str, payload: dict | None = None) -> dict:
    """
    Execute an authenticated HTTP request against the Nomba API.
    Injects the account header and bearer token automatically.
    """
    ...
    try:
        ...
        if response.status_code not in {200, 201}:
            ...
            raise HTTPException(
                status_code=status.HTTP_502_BAD_GATEWAY,
                detail=f"Nomba service communication error: {response.text}",
            )
        ...
        logger.error("Outbound connection failure for endpoint %s: %s", endpoint, error)
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail="Payment engine connectivity dropped. Try again.",
        ) from error
...
def create_checkout_order(
    amount_kobo: int,
    order_ref: str,
    school_subaccount_id: str | None = None,
    customer_email: str | None = None,
) -> str:
    """
    Create a Nomba online checkout order and return the generated checkout URL.

    Args:
        amount_kobo: Total amount in kobo (e.g. 150000 for ₦1,500.00).
        order_ref: Unique reference string for this order.
        school_subaccount_id: Optional Nomba sub-account to credit.
    ...
    payload = {"order": order_payload, "tokenizeCard": False}

    logger.info(
        "Initializing Nomba checkout. Reference=%s Amount=₦%s AccountId=%s",
        order_ref, amount_string, resolved_account_id,
    )
    ...
    if result.get("code") != "00":
        ...
        logger.error("Checkout initialization failed: %s", description)
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
...
def verify_transaction_by_id(transaction_id: str) -> dict:
    """Verify a transaction using Nomba's transactionRef lookup."""
...
def verify_checkout_transaction(order_reference: str) -> dict:
    """Verify a checkout transaction using the orderReference."""
